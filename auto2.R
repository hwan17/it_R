...
body <- dashboardBody(
  ...
  tabItems(
    ...
    tabItem(tabName = "histogram",
            sidebarPanel(
              selectInput("in_sel_hist_Var1","X Variable :", choices = NULL)
...
server <- function(input, output,session) {
  ...
  output$histogram <- renderPlotly({
    ...
    subplot(
      add_markers = plot_ly(data = table_in, x=~table_in[,input$in_sel_hist_Var1],type='histogram',name=input$in_sel_hist_Var1) %>%
        layout(yaxis=list(title=input$in_sel_hist_Var1))
